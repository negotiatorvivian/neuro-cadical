...
def gen_lbdp(td, cnf, is_train = True, logger = DummyLogger(verbose = True), dump_dir = None, dumpfreq = 50e3, timeout = None, clause_limit = 1e6):
    ...
    lbdp = LBDP(dp_id = name, is_train = np.array([is_train], dtype = "bool"), n_vars = np.array([n_vars], dtype = "int32"),
        n_clauses = np.array([n_clauses], dtype = "int32"), C_idxs = np.array(C_idxs), L_idxs = np.array(L_idxs), glue_counts = counts)
