...
class GNN1(nn.Module):
    def __init__(self, clause_dim, lit_dim, n_hops, n_layers_C_update, n_layers_L_update, n_layers_score, activation,
        ...
        self.V_score_drat = BasicMLP(input_dim = 2 * lit_dim,
            hidden_dims = [2 * lit_dim for _ in range(n_layers_score)], output_dim = 1, activation = activation,
            bias_at_end = True, p_dropout = 0.15)
        ...
        for T in range(self.n_hops):
            ...
            if self.average_pool:
                ...
                L_msg = L_msg / torch.max(L_neighbor_counts,
                    torch.ones(L_neighbor_counts.size()[0], L_neighbor_counts.size()[1], device = G.device))
...
class rl_GNN1(nn.Module):
    ...
    def forward(self, G):
        ...
        return (
            self.V_score(V), self.V_vote(V))  # return policy logits and value logits before averaging (for unbatching)
