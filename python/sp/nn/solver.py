...
class SATProblem(object):
    ...
    def setup_problem(self, data_batch, batch_replication):
        ...
        self._pos_mask_tuple = self._compute_graph_mask(self._graph_map, self._batch_variable_map,
            self._batch_function_map, (self._edge_feature == 1).squeeze(1).float())
    ...
    def _compute_batch_replication_map(self, batch_variable_map, batch_replication):
        ...
        if self._device.type == 'cuda':
            ...
            mask = torch.sparse.FloatTensor(ind, all_ones, torch.Size([batch_size * batch_replication, batch_size]),
                device = self._device)
    ...
    def _compute_variable_function_map(self, graph_map, batch_variable_map, batch_function_map, edge_feature):
        ...
        if self._device.type == 'cuda':
            mask = torch.cuda.sparse.FloatTensor(graph_map.long(), all_ones, torch.Size([variable_num, function_num]),
                device = self._device)
            ...
            mask = torch.sparse.FloatTensor(graph_map.long(), all_ones, torch.Size([variable_num, function_num]),
                device = self._device)
...
class PropagatorDecimatorSolverBase(nn.Module):
    ...
    def forward(self, init_state, graph_map, batch_variable_map, batch_function_map, edge_feature, meta_data,
            is_training = True, iteration_num = 1, check_termination = None, simplify = True, batch_replication = 1):
    ...
    def _forward_core(self, init_propagator_state, init_decimator_state, sat_problem, iteration_num, is_training,
        ...
        for _ in torch.arange(iteration_num, dtype = torch.int32, device = self._device):
            ...
            sat_problem._edge_mask = torch.mm(sat_problem._graph_mask_tuple[1],
                sat_problem._active_variables) * torch.mm(sat_problem._graph_mask_tuple[3],
    ...
    def _update_solution(self, prediction, sat_problem):
        ...
        if prediction[0] is not None:
            ...
            sat_problem._solution[sat_problem._active_variables[:, 0] == 1] = variable_solution[
    ...
    def _deduplicate(self, prediction, propagator_state, decimator_state, sat_problem):
        ...
        if prediction[1] is not None:
            ...
            function_prediction = (flag * prediction[1]).view(sat_problem._batch_replication, -1).sum(
                dim = 0).unsqueeze(1)
    ...
    def _local_search(self, prediction, sat_problem, batch_replication):
        ...
        sat_problem._edge_mask = torch.mm(sat_problem._graph_mask_tuple[1], sat_problem._active_variables) * torch.mm(
            sat_problem._graph_mask_tuple[3], sat_problem._active_functions)
    ...
    def get_init_state(self, graph_map, batch_variable_map, batch_function_map, edge_feature, graph_feat, randomized,
        ...
        if self._propagator is None:
            ...
            init_propagator_state = self._propagator.get_init_state(graph_map, batch_variable_map, batch_function_map,
                edge_feature, graph_feat, randomized, batch_replication)
...
class NeuralPropagatorDecimatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, edge_dimension, meta_data_dimension, propagator_dimension, decimator_dimension,
            mem_hidden_dimension, agg_hidden_dimension, mem_agg_hidden_dimension, prediction_dimension,
            variable_classifier = None, function_classifier = None, dropout = 0, local_search_iterations = 0,
            epsilon = 0.05):
        super(NeuralPropagatorDecimatorSolver, self).__init__(device = device, name = name,
            propagator = pdp_propagate.NeuralMessagePasser(device, edge_dimension, decimator_dimension,
                meta_data_dimension, propagator_dimension, mem_hidden_dimension, mem_agg_hidden_dimension,
                agg_hidden_dimension, dropout),
            decimator = pdp_decimate.NeuralDecimator(device, propagator_dimension, meta_data_dimension,
                decimator_dimension, mem_hidden_dimension, mem_agg_hidden_dimension, agg_hidden_dimension,
            ...
            predictor = pdp_predict.NeuralPredictor(device, decimator_dimension, prediction_dimension, edge_dimension,
                ...
                variable_classifier, function_classifier), local_search_iterations = local_search_iterations,
            epsilon = epsilon)
...
class NeuralSurveyPropagatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, edge_dimension, meta_data_dimension, decimator_dimension, mem_hidden_dimension,
            agg_hidden_dimension, mem_agg_hidden_dimension, prediction_dimension, variable_classifier = None,
...
class SurveyPropagatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, tolerance, t_max, local_search_iterations = 0, epsilon = 0.05):
        super(SurveyPropagatorSolver, self).__init__(device = device, name = name,
            ...
            decimator = pdp_decimate.SequentialDecimator(device, message_dimension = (3, 1),
                scorer = pdp_predict.SurveyScorer(device, message_dimension = 1, include_adaptors = False),
...
class WalkSATSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, iteration_num, epsilon = 0.05):
        super(WalkSATSolver, self).__init__(device = device, name = name, propagator = None, decimator = None,
            predictor = pdp_predict.IdentityPredictor(device = device, random_fill = True),
...
class ReinforceSurveyPropagatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, pi = 0.1, decimation_probability = 0.5, local_search_iterations = 0,
            epsilon = 0.05):
        super(ReinforceSurveyPropagatorSolver, self).__init__(device = device, name = name,
            propagator = pdp_propagate.SurveyPropagator(device, decimator_dimension = 1, include_adaptors = False,
                pi = pi), decimator = pdp_decimate.ReinforceDecimator(device,
...
class NeuralSequentialDecimatorSolver(PropagatorDecimatorSolverBase):
    ...
    def __init__(self, device, name, edge_dimension, meta_data_dimension, propagator_dimension, decimator_dimension,
            mem_hidden_dimension, agg_hidden_dimension, mem_agg_hidden_dimension, classifier_dimension, dropout,
            tolerance, t_max, local_search_iterations = 0, epsilon = 0.05):
        super(NeuralSequentialDecimatorSolver, self).__init__(device = device, name = name,
            propagator = pdp_propagate.NeuralMessagePasser(device, edge_dimension, decimator_dimension,
                meta_data_dimension, propagator_dimension, mem_hidden_dimension, mem_agg_hidden_dimension,
                agg_hidden_dimension, dropout),
            decimator = pdp_decimate.SequentialDecimator(device, message_dimension = (3, 1),
                scorer = pdp_predict.NeuralPredictor(device, decimator_dimension, 1, edge_dimension,
                    meta_data_dimension, mem_hidden_dimension, agg_hidden_dimension, mem_agg_hidden_dimension,
                    variable_classifier = util.PerceptronTanh(decimator_dimension, classifier_dimension, 1),
                    function_classifier = None), tolerance = tolerance, t_max = t_max),
            predictor = pdp_predict.IdentityPredictor(device = device, random_fill = True),
            local_search_iterations = local_search_iterations, epsilon = epsilon)
