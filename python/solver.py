...
def cadical_fn(cnf_path, refocus = False, query_interval = None, model = None, verbose = False, refocus_scale = 1e7,
        random_refocus = False, cpu_lim = None, irrlim = 10e6, seed = None, refocus_init_time = 60, refocus_base = 5000,
...
class StatHolder:
    def __init__(self, program_name, prog_args, prog_alias, benchmark, m_options = dict()):
        self.preamble = {
            "program": program_name, "prog_args": prog_args, "prog_alias": prog_alias, "benchmark": benchmark
        }
    ...
    def store_result(self, instance_name, result_dict):
        if result_dict.get("errored_out", False):
            self.stats[instance_name] = {
                "status": False, "rtime": 0, "decisions": 0, "propagations": 0, "mem_used": 0, "result": None,
                "conflicts": 0, "glr": 0, "errored_out": True
            }
        else:
            try:
            ...
            self.stats[instance_name] = {
                ...
                "mem_used": result_dict["mem_used"], "result": result_dict["result"],
                "conflicts": result_dict["conflicts"], "glr": glr, "num_queries": result_dict["num_queries"],
...
def test_harness(cnf_dir, m_options, statholder_options, out_path, pool, exts = ["cnf", "gz"], forbidden = [],
        solver = "cadical"):
    if solver == "cadical":
